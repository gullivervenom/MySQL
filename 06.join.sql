-- * join *
-- 두 개 이상의 테이블을 관련 있는 컬럼을 통해 결합(관계를 맺는)하는데 사용한다. 
-- 두 개 이상의 테이블은 결합을 하기 위해서는 반드시!!! 연관 있는 컬럼이 존재해야 하며
-- 연관 있는 컬럼을 통해서 join 이 된 테이블들의 컬럼을 모두 사용할 수 있다.

-- join 시작 전 별칭 정리

-- as(alias) 별칭
-- sql 문(퀴리문)의 컬럼 또는 테이블에 별칭을 달 수 있다.
...
-- using을 사용해서 연관있는 컬럼을 join 할 수 있다.
select
    a.menu_name,
    b.category_name
from
...
-- right join
-- 두 번째(오른쪽) 테이블의 모든 레코드와 첫 번쨰(왼 쪽)테이블에서 
...
from
	...
	on
    a.ref_tbl_category = b.category_code;
    
-- 서로 다른 테이블 간의 데이터를 조회하고 싶을 때는 join 을 사용한다. 
-- join 을 수행하는 단계를 고려할 때는
-- 1. 테이블과 테이블이 연관이 되어 있는 지 확인 
-- 2. 연결이 되어 있다고 하면 어떤 컬럼으로 연결이 되어 있는 지 확인 
